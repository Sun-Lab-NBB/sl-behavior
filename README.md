...
uncompressed .feather files that are later integrated into unified project datasets using the assets from the
[sl-forgery library](https://github.com/Sun-Lab-NBB/sl-forgery).

The library is designed to be called via the bindings in the sl-forgery library and run on remote compute servers.
