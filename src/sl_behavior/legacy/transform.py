...
def add_timestamp_values(
    ...
    Args:
        df (pd.DataFrame):
            DataFrame to which new values will be added (must have a "time_us" column).
        timestamp_df (pd.DataFrame):
            Timestamped DataFrame with fields to assign (must have a "time_us" column, sorted ascending).
        fields (List[str]):
    ...
    timestamp_df = timestamp_df.sort_values("time_us")
    ...
    ind = np.array(timestamp_df["time_us"].searchsorted(df["time_us"]) - 1)
...
def add_ranged_timestamp_values(
    ...
    Args:
        df (pd.DataFrame):
            The DataFrame to be enriched, must have a "time_us" column.
        timestamp_df (pd.DataFrame):
