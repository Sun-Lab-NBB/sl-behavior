...
def _process_runtime_data(
    ...
    """Extracts the acquisition-system-specific data from the log generated during a data acquisition runtime and saves
    the extracted data as Polars DataFrame .feather files.

    This function works similar to camera log extraction and hardware module log extraction functions. The key
    difference is that this function contains additional arguments to support extracting the data generated by different
    data acquisition systems used in the Sun lab.

    Args:
        log_path: The path to the .npz archive containing the acquisition system data to parse.
        output_directory: The path to the directory where to save the extracted data as .feather files.
        experiment_configuration: The ExperimentConfiguration class for the processed runtime, if the processed runtime
            is an experiment.
    """
