"""Provides assets for extracting acquisition-system and runtime task data from .npz log archives generated by data
acquisition systems used in the Sun lab.
"""
...
def _decompose_sequence_numba_flat(
    ...
    Notes:
        Uses numba-acceleration to speed up decomposition.
...
def _decompose_cue_sequence_into_trials(
    ...
    Notes:
        Convenience wrapper around the _decompose_multiple_cue_sequences_into_trials() function for runtimes that only
        used a single wall cue sequence. Since multiple sequences are only present in runtimes that encountered issues,
        typically used during most data processing runtimes.
...
def _process_trial_sequence(
    ...
    Notes:
        Processes the trial sequences generated by the _decompose_cue_sequence_into_trials() and
        _decompose_multiple_cue_sequences_into_trials() functions. The metadata extracted is used to support
        trial-based data analysis in the sl-forgery library.
...
def _extract_mesoscope_vr_data(
    ...
    Notes:
        Specifically designed to process the data logged by the Mesoscope-VR data acquisition system. Does not work
        for any other Sun lab data acquisition system.
...
def process_runtime_data(session_path: Path) -> None:
    ...
    Notes:
        Processes the log archives generated by the Mesoscope-VR data acquisition system used in the Sun lab. Assumes
        that the data was logged using the assets from the sl-experiment library and that the acquisition system log
        file uses the source ID '1'.
