    ...
    with ProcessPoolExecutor() as executor:
        ...
        for file in compressed_files:
            ...
            if file.stem == "101_log":
                ...
                # Valve
                if (
                    hardware_configuration.valve_nonlinearity_exponent is not None
                    and hardware_configuration.valve_scale_coefficient is not None
                ):
                    futures.add(
                        executor.submit(
                            parse_valve_data,
                ...
                # Screens
                if hardware_configuration.screens_initially_on is not None:
                    futures.add(
                        executor.submit(
                            parse_screen_data,
                            file,
                            behavior_data_directory,
                            hardware_configuration.screens_initially_on,
                        )
                    )
            ...
            if file.stem == "152_log":
                ...
                if hardware_configuration.recorded_mesoscope_ttl:
