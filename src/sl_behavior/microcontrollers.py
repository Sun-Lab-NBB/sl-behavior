"""Provides assets for extracting behavior data from .npz log archives generated by the microcontrollers (hardware
modules) used in the Sun lab.
"""
...
class _ParseTask(TypedDict):
    """Enforces static typing while supporting parallel microcontroller data processing."""
...
def _parse_brake_data(
    ...
    Notes:
        Assumes that the brake was used in the absolute force mode. Currently does not support extracting variable
        brake power data.
    """
    event_data = extracted_module_data.event_data
...
def _parse_screen_data(extracted_module_data: ExtractedModuleData, output_file: Path, *, initially_on: bool) -> None:
    ...
    Notes:
        Works similarly to the TTLModule extraction method. ScreenInterface is essentially a group of 3 TTLModules.
    """
    event_data = extracted_module_data.event_data
...
def _parse_module_data(
    ...
    **kwargs: bool | np.float64 | np.uint16 | int,
    ...
    Parses hardware module data in parallel using the provided parsing function.
...
def process_microcontroller_data(
    ...
    Processes the log archives generated by the microcontrollers used in the Sun lab. Assumes that the data was logged
    using the assets from the ataraxis-communication-interface library.
