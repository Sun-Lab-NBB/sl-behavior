...
def _process_module_message_batch(
        arguments: tuple[Path, list[str], np.uint64, tuple[tuple[int, int], ...]]
    ...
    Args:
        arguments: A tuple of four elements (log_path, file_names, onset_us, module_type_id). The first element stores
            the path to the processed log file. The second element stores the names of the individual message .npy
            files stored in the target archive. The third element stores the onset of the data acquisition, in
            microseconds elapsed since UTC epoch onset. The fourth element stores the module type and ID codes to
            extract.
    ...
    log_path, file_names, onset_us, module_type_id = arguments
        ...
        for item in file_names:
            ...
            if len(payload) > 5:
                ...
                if isinstance(prototype, np.ndarray):
                    ...
                    # For parallel processing, we collect errors but don't raise them immediately
                    data = None  # Mark as error case
...
def extract_logged_hardware_module_data(
    ...
    Notes:
        ...
        regardless of the specified worker count to avoid multiprocessing overhead.
    ...
    if n_workers == 1 or len(messages_to_process) < 2000:
        ...
        for i, batch in enumerate(chunk_iterable(messages_to_process, n_workers * 4)):
            if batch:
                batches.append((log_path, batch, onset_us, module_type_id))
        ...
        with ProcessPoolExecutor(max_workers=n_workers) as executor:
            ...
            # Submits all tasks
            future_to_index = {
                executor.submit(_process_module_message_batch, batch): idx
                for idx, batch in zip(batch_indices, batches)
            }

            # Collects results while maintaining message order. This also propagates processing errors to the caller process.
        ...
        # Processes results from each batch in order to maintain chronological ordering
...
timer.reset()
extract_logged_hardware_module_data(log_path=Path("/media/Data/MaalstroomicFlow/15/2025-07-18-12-00-40-459354/raw_data/behavior_data/203_log.npz"), module_type_id=((2, 1),), n_workers=30)
print(timer.elapsed)
